...
function CompetitiveSection() {
  ...
  return (
    <section id="competitive" data-screen-label="13 Competitive">
...
function TakeSection() {
  return (
    <section id="take" className="sec--red" data-screen-label="14 Take">
      <div className="section__inner">
        <SectionHead
          num="12"
          kicker="The UM Take · Recommendations + Predictions"
          title={<>How We'd Walk<br />Into The Room.</>}
          lede="Two lists. Five recommendations for how to approach 2026 buys. Six predictions for what we expect to happen in the room itself. Everything above built to this."
        />
          ...
          <div style={{background: 'var(--ink)', color: 'var(--paper)'}}>
            {TAKE_PREDICTIONS.map((p, i) => (
              <div key={p.n} style={{
                ...
                padding: '24px 32px',
                alignItems: 'center',
                borderTop: i === 0 ? 'none' : '1px solid var(--muted)',
              }}>
                <div style={{fontFamily: 'var(--f-display)', fontSize: 36, color: 'var(--um-red)'}}>{p.n}</div>
                <div style={{fontFamily: 'var(--f-serif)', fontSize: 18, lineHeight: 1.45, textWrap: 'pretty'}}>{p.body}</div>
                <div style={{textAlign: 'right'}}>
                  <span style={{
                    display: 'inline-block',
                    padding: '6px 10px',
                    border: `1px solid ${p.confidence === 'HIGH' ? 'var(--um-red)' : 'var(--muted)'}`,
                    color: p.confidence === 'HIGH' ? 'var(--um-red)' : 'var(--paper)',
        ...
        <div style={{marginTop: 80, padding: '40px 0', borderTop: '4px solid var(--paper)', borderBottom: '2px solid var(--paper)', textAlign: 'center'}}>
          <div style={{fontFamily: 'var(--f-display)', fontSize: 'clamp(48px, 8vw, 104px)', lineHeight: 0.9, textTransform: 'uppercase', letterSpacing: '-0.03em'}}>
            FULL STREAM<br />AHEAD.
          </div>
        </div>
      </div>
    </section>
...
function Colophon() {
  return (
    <footer className="colophon" data-screen-label="15 Colophon">
